...
# set the folder of the downloaded pretrained model (MobileNetV2)
...
# training set, shuffle sets to be true
train_generator = train_datagen.flow_from_directory(
    data_dir, subset="training", shuffle=True, **dataflow_kwargs)

# validation set, shuffle sets to be false
...
# add a preprocessing layer of random_crop
...
model = tf.keras.Sequential([
    ...
    # add the random crop layer here,  crop before inference time
    random_crop,

    # pretraind model of MobileNetV2
    hub.KerasLayer(MODULE_HANDLE, trainable=do_fine_tuning),
    tf.keras.layers.Dropout(rate=0.2), # dropout layer

    # last classification layer in the model that will be trained
...
callbacks = [tf.keras.callbacks.CSVLogger('./training.csv', separator=",", append=False), # save performance to csv
             tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=1, min_lr=1e-8, verbose=1, mode='auto') # reduce lr if necessary
...
#save the model in the result folder
